    ...
    <main>
      ...
      <MemberlistBox closeCheck={memberlist}>
        {memberlist
          ? memberlist.map((data) => {
              if (
                data["info"]["basic"]["username"] &&
                data["info"]["basic"]["title"] &&
                data["info"]["basic"]["username"] &&
                data["info"]["basic"]["welcome"]
              ) {
                let id = data["id"];
                let tags = data["info"]["detail"]["keyword"];
                return (
                  <MemberCard to={`/membership/${id}`}>
                    <Headshot>
                      <img src={data["info"]["basic"]["headshot"]}></img>
                    </Headshot>
                    <MemberCardContent>
                      <CardContentName>
                        {data["info"]["basic"]["username"]}
                      ...
                      <CardTagsBox>
                        {tags.map((tag, index) => {
                          if (index < 5) {
                            return (
                              <div className="memberlist-card-tag">
                                <p>{tag["tag"]}</p>
                              </div>
                            );
                          }
