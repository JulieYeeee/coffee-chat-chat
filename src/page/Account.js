...
import {
  ...
  HeadshotLabel,
  BasicInfoBox,
} from "../component/style/Account.styled";
import {
  AccountLinkBox,
  SingleLink,
  AccountIntroBox,
...
const Account = () => {
  ...
  const getAfterElement = (childrenNodeArray, cursorX) => {
    const newDraggableElements = childrenNodeArray.filter(
      (child) => child.children[0].className.indexOf("dragging") == -1
    );
    return newDraggableElements.reduce(
      (start, afterElement, index) => {
        const afterElementInfo = afterElement.getBoundingClientRect();
        const offset =
          cursorX - afterElementInfo.left - afterElementInfo.width / 2;
  ...
  return (
    <main>
      <StoreNotification closeCheck={linkNotificationClose}>
        ...
        <p>資料儲存成功🆗</p>
        <input
          value={"https://coffee-chat-together.web.app" + membershipLink}
          readOnly
        ></input>
        <div>
          <span onClick={copyMembershipLink}>複製網址</span>
          <Link to={membershipLink} target="_blank">
            前往頁面
          </Link>
        </div>
      </StoreNotification>
      ...
      <AccountForm closeCheck={userDatatRef.current}>
        ...
        <AccountButton type="submit" onClick={addUserData}>
          填寫完成，建立個人頁面
        </AccountButton>
      </AccountForm>
    </main>
