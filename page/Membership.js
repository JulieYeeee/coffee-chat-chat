...
const Membership = () => {
  ...
  let [projects, setProjects] = useState([
    { cover: null, type: null, content: null, link: null },
    { cover: null, type: null, content: null, link: null },
    { cover: null, type: null, content: null, link: null },
  ]);
  ...
  const setMemberData = async (userData) => {
    ...
    closeRef.current = 0;
    const db = getFirestore(firebase);
    const getConsultAccountQuery = query(
      collection(db, "user"),
      where("user.email", "==", userData["user"]["email"])
    );
  ...
  const buildAsk = async (e) => {
    ...
    if (account) {
      const checkQuery = query(
        collection(db, "pre-order"),
        where("account", "==", account),
        where("consultantEmail", "==", memberEmail),
        where("payment", "==", null)
      );
    ...
    if (userDocSnap.exists()) {
      let newUsername = JSON.parse(
        JSON.stringify(userDocSnap.data()["basic"]["username"])
      );
          ...
          <MemberDataRight>
            <Askform>
              ...
              <div className="ask-rule-box">
                ...
                <p className="ask-rule">
                  不得詢問個人隱私之問題，若因提問不當分享者有權婉轉回覆。請盡量詢問分享者可分享領域，若因提問超出分享範圍，可能導致您收不到良好回覆。
                </p>
              </div>
            </Askform>
          </MemberDataRight>
